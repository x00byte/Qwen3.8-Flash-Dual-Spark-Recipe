...
class Qwen3_8FlashNextPLEFp8EmbeddingMethod(QuantizeMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # Note: the single global PLE scale is loaded as a buffer by
        # Qwen3_8FlashNextNGramEmbedding.load_weights (register_buffer), so it
        # must not also be registered as a parameter here or loading would
        # raise "attribute 'weight_scale' already exists".
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            if name.startswith(shard_prefix) and name.endswith(".weight"):
                ...
                continue
            # Some ModelOpt configs omit this PLE embedding from the generic
            # quantization map even though its checkpoint shards are FP8.
            # Preserve the exported scale so forward() can dequantize the
            # looked-up values (see _get_embedding_weight_scale below).
